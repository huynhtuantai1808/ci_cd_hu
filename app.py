...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not session.get('is_admin'):
            # Thay vì redirect, trả về lỗi 403 Forbidden cho API calls
            if request.path.startswith('/api/'):
                return jsonify({"success": False, "message": "Yêu cầu quyền Admin."}), 403
...
def webhook(project_id):
    ...
    if project.get('gitlab_webhook_secret') != gitlab_token: return jsonify({"status": "error", "message": "Invalid secret token"}), 403
    print(f"Webhook received for project: {project_id}. Starting deployment...")
    success, log = trigger_deployment(project_id)
    if success: return jsonify({"status": "success"})
    else: return jsonify({"status": "error", "message": "Deployment failed", "log": log}), 500

# --- Trang Admin và API quản lý User (yêu cầu Admin) ---
# ... (Không thay đổi) ...
@app.route('/admin')
@login_required
@admin_required
...
def add_user():
    ...
    if not username or not password: return jsonify({"success": False, "message": "Tên đăng nhập và mật khẩu là bắt buộc."}), 400
    users_data = get_data('users.json')
    if username in users_data['users']: return jsonify({"success": False, "message": f"Tên đăng nhập '{username}' đã tồn tại."}), 409
    users_data['users'][username] = { "id": username, "username": username, "password_hash": generate_password_hash(password), "is_admin": is_admin }
    if save_data('users.json', users_data): return jsonify({"success": True, "message": "Thêm người dùng thành công."})
    else: return jsonify({"success": False, "message": "Lỗi khi lưu dữ liệu người dùng."}), 500
...
def update_user(user_id):
    data = request.json
    users_data = get_data('users.json')
    if user_id not in users_data['users']: return jsonify({"success": False, "message": "Không tìm thấy người dùng."}), 404
    ...
    if 'is_admin' in data:
        if user_id == session['user_id'] and not data['is_admin']: return jsonify({"success": False, "message": "Bạn không thể tự tước quyền Admin của chính mình."}), 403
        user['is_admin'] = data['is_admin']
    if save_data('users.json', users_data): return jsonify({"success": True, "message": "Cập nhật người dùng thành công."})
    else: return jsonify({"success": False, "message": "Lỗi khi lưu dữ liệu người dùng."}), 500
@app.route('/api/users/<user_id>', methods=['DELETE'])
@login_required
@admin_required
def delete_user(user_id):
    if user_id == session['user_id']: return jsonify({"success": False, "message": "Bạn không thể xóa chính mình."}), 403
    users_data = get_data('users.json')
    if user_id not in users_data['users']: return jsonify({"success": False, "message": "Không tìm thấy người dùng."}), 404
    del users_data['users'][user_id]
    if save_data('users.json', users_data): return jsonify({"success": True, "message": "Xóa người dùng thành công."})
    else: return jsonify({"success": False, "message": "Lỗi khi lưu dữ liệu người dùng."}), 500
